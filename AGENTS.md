...
For ship and scout tasks, tmux/herdr/zellij/cmux create a runtime endpoint and lease a worktree with `treehouse get --lease` (recording its `lease_holder=` owner token; a treehouse without `--lease` falls back to bare `treehouse get` with pane-cwd polling and no `lease_holder=`); Orca creates an Orca-owned worktree, validates it, then creates the terminal. In all cases, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
Independently, when the task meta recorded a `lease_holder=` token, teardown confirms the worktree's live treehouse lease state first and fails closed: it returns the worktree only when `treehouse status` positively shows it held by exactly that token or explicitly unheld, and otherwise refuses (`REFUSED: ...`) - a different home's holder, a worktree row missing from status, or a status error all refuse, so this home never returns a worktree another firstmate home is live in.
A missing `lease_holder=` keeps the pre-lease behavior, and `--force` never bypasses the guard because it discards only this task's work.
