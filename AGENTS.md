...
Independently, when the task meta recorded a `lease_holder=` token, teardown reads the worktree's live treehouse lease holder first and refuses (`REFUSED: worktree ... is leased by a different holder`) when a different home holds it, so this home never returns a worktree another firstmate home is live in; a missing `lease_holder=` or an unheld worktree skips that guard, and `--force` never bypasses it because it discards only this task's work.
