...
As a fail-closed backstop, both scripts refuse to run when their own home carries the `.fm-secondmate-home` marker but `FM_HOME` was lost - unset with no `FM_*_OVERRIDE` in play - because resolution would otherwise silently fall back to that home and leak state into it; a primary home, an explicit `FM_HOME`, or any operational override leaves the guard inert (`bin/fm-home-lib.sh`).
A `--secondmate` launch also exports `FM_HOME` into the pane shell before the agent starts, in addition to the launch-command prefix, so a harness that scrubs the launch-command environment still carries `FM_HOME` into the agent's own shell and its tool subshells (verified for `claude`; see [`docs/fm-home-propagation.md`](fm-home-propagation.md)).
